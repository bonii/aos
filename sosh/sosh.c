...
static int thrash(int argc, char **argv) {
  ...
  for (size_t i = 0; i < buf_size; ++i) {
    buf[i] = i % 26 + 'A';
    if (buf[i] != i % 26 + 'A') {
      printf("Error on write at index %d. Should be %c, is %c\n",
	     i, i % 26 + 'A',
                    buf[i]
	     );
    }
  ...
  for (size_t i = 0; i < buf_size; ++i) {
    if (buf[i] != i % 26 + 'A') {
      printf("Error at index %d. Should be %c, is %c\n", i,
	     i % 26 + 'A', buf[i]);
      if (++errors >= 10) {
...
cp(int argc, char **argv)
{
    ...
    while( (num_read = read( fd, buf, 8*BUF_SIZ) ) > 0 ) {
        long start = time_stamp();
	num_written = write(fd_out, buf, num_read);
        long end = time_stamp();
        if(0) printf("Time taken to write %d bytes %ld\n",num_written,(end-start));
    }
    if( num_read == -1 || num_written == -1 )
    {
	    printf( "error on cp\n" );
...
time(int argc, char **argv) {
  ...
  printf("Time since booting is: %ld us\n",time);
  return 0;
}

static int
sleep_func(int argc, char **argv) {
  if(argc >= 2) {
    sleep(atoi(argv[1]));
  }
  return 0;
